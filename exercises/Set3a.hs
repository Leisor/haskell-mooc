...
firstHalf str = if even (length str)
                then take (div (length str) 2) str
                else take (div (1+length str) 2) str
...
powers :: Int -> Int -> [Int]
powers k max = takeWhile (<= max) (powersOf k)
powersOf k = iterate (*k) 1
...
joinToLength len xs = [i | i <- [x ++ y | x <- xs, y <- xs], length i == len]
...
multiCompose fs x = go (reverse fs) x
    where
        go [] x = x
        go (f:fs) x = go fs (f x)
...
interpreter commands = execute commands 0 0

execute [] x y = [] 
execute ("up":xs) x y     = execute xs x (y + 1) 
execute ("down":xs) x y   = execute xs x (y - 1)
execute ("right":xs) x y  = execute xs (x + 1) y
execute ("left":xs) x y   = execute xs (x - 1) y
execute ("printY":xs) x y = show y : execute xs x y
execute ("printX":xs) x y = show x : execute xs x y
