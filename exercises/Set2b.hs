...
oddFactorial 0 = 1
oddFactorial 1 = 1
--oddFactorial (even n) = (n - 1) * oddfactorial(n-2)
oddFactorial n = if even n
                 then (n-1) * oddFactorial (n-2)
...
countdown' n result = if (n == 0)
                      then result
                      else countdown' (n-1) (result++(show n ++ "... "))
...
smallestDivisor n = if (n `mod` 2 == 0)
                    then 2
                    else smallestDivisor' n 2
smallestDivisor' n k = if (n `mod` k == 0)
               then k
...
biggestPrimeAtMost n = if isPrime n == True then n else biggestPrimeAtMost (n-1)
