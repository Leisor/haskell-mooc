...
countHelper y count = maybe defaultValue isNotNothing y 
    where
        defaultValue = 1 + count
        isNotNothing _ = count
...
maxHelper y max = if y > max then y else max
...
slHelper y (sum, count) = (sum + y, count + 1)
...
concatHelper :: [a] -> [a] -> [a]
concatHelper y concatList = y ++ concatList
