...
paintSolid color (Shape s) (Picture p) =
    Picture (\coord -> if s coord then color else p coord)
------------------------------------------------------------------------------
...
paint (Picture p) (Shape s) (Picture b) =
    Picture (\coord -> if s coord then p coord else b coord)
------------------------------------------------------------------------------
...
instance Transform Blur where
  apply Blur (Picture p) =
    Picture (\(Coord x y) ->
      let [Color r1 g1 b1, Color r2 g2 b2, Color r3 g3 b3, Color r4 g4 b4, Color r5 g5 b5] =
            [p (Coord x y), p (Coord (x+1) y), p (Coord (x-1) y), p (Coord x (y+1)), p (Coord x (y-1))]
