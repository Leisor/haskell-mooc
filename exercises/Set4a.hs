...
allEqual xs = if length (nub xs) < 2 then True else False
...
longest (x:xs) = longest' x xs
    where
        longest' currentMax [] = currentMax
        longest' currentMax (y:ys)
            | length y > length currentMax                    = longest' y ys
            | length y == length currentMax && y < currentMax = longest' y ys
            | otherwise                                       = longest' currentMax ys
...
freqs xs = freqs' xs Map.empty
    where
        freqs' [] freqsMap = freqsMap
        freqs' (x:xs) freqsMap =
            case Map.lookup x freqsMap of
                Nothing -> freqs' xs (Map.insert x 1 freqsMap)
                Just count -> freqs' xs (Map.insert x (count + 1) freqsMap)
...
transfer from to amount bank =
    case (Map.lookup from bank, Map.lookup to bank) of
        (Just sum1, Just sum2)
            | amount >= 0 && sum1 >= amount ->
                Map.insert from (sum1 - amount) (Map.insert to (sum2 + amount) bank) 
        _                                   -> bank
...
maxIndex arr = fst (foldr compMax (head pairs) pairs)
    where
        pairs = assocs arr
        compMax (i1, v1) (i2, v2)
            | v1 > v2 = (i1, v1)
