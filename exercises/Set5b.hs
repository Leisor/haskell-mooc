...
cull _ Empty = Empty
cull val (Node x left right) 
    | x == val  = Empty
    | otherwise = Node x (cull val left) (cull val right)
...
isOrdered (Node x left right) = 
    allValues (<x) left && allValues (>x) right && isOrdered left && isOrdered right
...
set (StepL:steps) val (Node x left right) = 
    Node x (set steps val left) right
set (StepR:steps) val (Node x left right) = 
    Node x left (set steps val right)
